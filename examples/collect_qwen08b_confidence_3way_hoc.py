"""Collect Qwen3.5-0.8B 3-way confidence features for ALL hoc (doc, query) pairs.

Same 3-way prompt + logprob extraction as collect_qwen08b_confidence_3way_opp115.py;
hoc uses the same merged_df.csv / query_name_mapping.csv / pairs_ground_truth.csv
layout as opp115.

Output: examples/predictions/zeroshot_all_queries/Qwen3.5-0.8B_confidence_3way_hoc.csv

Provides the 0.8B log-odds (logprob_true - logprob_false) consumed by the
Stretto baseline (tools/stretto).
"""
...
DATASET_DIR = Path(__file__).resolve().parent.parent / "datasets" / "hoc"
...
def main():
    ...
    output_file = OUTPUT_DIR / "Qwen3.5-0.8B_confidence_3way_hoc.csv"
