"""Collect Meta-Llama-3.1-8B-Instruct 3-way confidence features for ALL opp115 (doc, query) pairs.

Same 3-way prompt + logprob extraction as collect_qwen08b_confidence_3way_opp115.py,
with the small model swapped to the 8B (served on :8106) — provides an 8B
log-odds signal for the Stretto baseline (tools/stretto).

Output: examples/predictions/zeroshot_all_queries/Llama-3.1-8B-Instruct_confidence_3way_opp115.csv
...
def main():
    ...
    client.chat.completions.create(
        ...
        extra_body={"chat_template_kwargs": {"enable_thinking": False}})
    print(f"[OK] connected to 8B at {BASE_URL}")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    output_file = OUTPUT_DIR / "Llama-3.1-8B-Instruct_confidence_3way_opp115.csv"
